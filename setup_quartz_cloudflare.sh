...
for env_file in "$HOME/.devops-lab4.env" ".env"; do
  if [ -f "$env_file" ]; then
    set -a
...
# ====== EDIT ======
export GH_USER="${GH_USER:-ismahamat}"
export REPO="${REPO:-devops-lab4}"
export PROJ="${PROJ:-devops-lab4}"
...
export SITE_DIR="$HOME/devops-lab4/$REPO"
...
log_step "1) Ensuring Node.js 22"
if [[ "$(node -v 2>/dev/null)" == v22* ]]; then
  echo "Node.js 22 already active."
else
  # Detect NVM_DIR (standard or Homebrew)
  if [ -d "$HOME/.nvm" ]; then
    ...
    echo "nvm missing; install nvm or node@22 first." >&2
    exit 1
  fi

  if [ -s "$NVM_DIR/nvm.sh" ]; then
    . "$NVM_DIR/nvm.sh"
    nvm use 22 2>/dev/null || { nvm install 22 && nvm alias default 22 && nvm use 22; }
  else
...
if ! has gh; then
  if has brew; then brew install gh;
  elif has apt-get; then sudo apt-get update && sudo apt-get install -y gh jq;
  else echo "Install gh manually"; exit 1; fi
fi
# Wrangler is used via npx to avoid global permission issues
if ! has python3; then
  if has brew; then brew install python;
  elif has apt-get; then sudo apt-get install -y python3 python3-pip;
  else echo "Install python3"; exit 1; fi
fi
...
case "$project_status" in
  ...
  404) npx wrangler pages project create "$PROJ" --production-branch main || echo "Warn: npx wrangler create failed" >&2;;
  *) echo "Warn: verify project (HTTP $project_status): $(cat "$project_tmp")" >&2; npx wrangler pages project create "$PROJ" --production-branch main || true;;
esac
rm -f "$project_tmp"
